...
export default function Brands() {
  ...
  return (
    <div className="catogries mt-20 md:mt-14 px-4 md:px-20"> 
      <h1 className="text-5xl text-center  text-red-600 font-semibold pt-10 mb-5">
        Our Brands
      </h1>
      {brands.length > 0 ? (
          ...
          {brands.map((brand) => (
            <div key={brand._id} className="w-full lg:w-1/4 md:w-1/3 sm:w-1/2 p-3">
              <div className="brand rounded-lg shadow-2xl overflow-hidden cursor-pointer">
                <img
  className="h-[200px] object-cover transform transition-transform duration-300 hover:scale-110 "
  src={brand.image}
                  alt={brand.name}
                />
                <h2 className="m-3 text-red-600 text-2xl font-bold">
                  {brand.name}
                </h2>
              </div>
            </div>
